    ...
    <div className="relative w-full h-full bg-slate-100 rounded-3xl overflow-hidden border border-slate-200 shadow-inner">
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className="w-full h-full p-20 flex flex-wrap content-start">
           <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 border-4 border-dashed border-emerald-200/50 rounded-full animate-pulse" />
        </div>
      </div>
      ...
      <div className="absolute top-6 left-1/2 -translate-x-1/2 bg-white/90 backdrop-blur px-4 py-2 rounded-full shadow-lg border border-slate-200 z-10 flex items-center gap-3">
        <span className="text-xs font-bold text-slate-600 uppercase tracking-widest">Active Region</span>
        <div className="h-4 w-px bg-slate-300" />
        <span className="text-sm font-bold text-emerald-600">Central & Nairobi</span>
      </div>
      ...
      {properties.map((prop, idx) => {
        ...
        return (
          ...
          >
            ...
            >
              <div className={`px-2 py-1 rounded-lg text-[10px] font-bold shadow-lg border border-white transition-colors ${selectedId === prop.id ? 'bg-emerald-600 text-white' : 'bg-white text-slate-900 group-hover:bg-emerald-50'}`}>
                KES {Math.round(prop.price / 1000)}k
              </div>
              <div className={`w-4 h-4 rounded-full border-2 border-white shadow-lg transition-colors ${selectedId === prop.id ? 'bg-emerald-600' : 'bg-emerald-400 group-hover:bg-emerald-500'}`} />
            </button>
          </div>
        );
      })}
      ...
      <div className="absolute bottom-6 left-6 bg-white p-4 rounded-2xl shadow-xl border border-slate-200 max-w-[200px] z-10">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] text-slate-500">Available Listings</span>
          </div>
          <div className="flex items-center gap-2">
